...
`,H_=S.div`
  ...
  position: absolute;
  width: 10px;
`,F_=({children:e})=>{const t=H.useRef(null),n=H.useRef(null),r=H.useRef(null),i=H.useRef(null),[o,l]=H.useState(20),[a,s]=H.useState(null),[u,h]=H.useState(0),[f,m]=H.useState(!1);function k(g,w){const{clientHeight:$,scrollHeight:C}=g;l(Math.max($/C*w,20))}const _=H.useCallback(g=>{if(!f&&t.current&&r.current&&g.target===t.current){const{scrollTop:w,scrollHeight:$}=t.current,{clientHeight:C}=n.current;let b=+w/+$*C;b=Math.min(b,C-o);const T=r.current;T.style.top=`${b}px`}},[f,o]);H.useEffect(()=>{const g=t.current;if(g)return g.addEventListener("scroll",_,{passive:!0}),g.addEventListener("touchmove",_,{passive:!1}),()=>{g.removeEventListener("scroll",_),g.removeEventListener("touchmove",_)}},[_]);const x=H.useCallback(g=>{g.preventDefault(),g.stopPropagation();const{current:w}=n,{current:$}=t;if(w&&$){const{clientY:C}=g,I=g.target.getBoundingClientRect().top,M=-(o/2),R=(C-I+M)/w.clientHeight,_e=Math.floor(R*$.scrollHeight);$.scrollTo({top:_e,behavior:"smooth"})}},[o]),E=H.useCallback(()=>{if(!f){if(!t.current||!n.current||!r.current)return;const{scrollTop:g,scrollHeight:w}=t.current,{clientHeight:$}=n.current;let C=+g/+w*$;C=Math.min(C,$-o);const b=r.current;b.style.top=`${C}px`}},[f,o]),p=H.useCallback(g=>{g.stopPropagation(),s(g.touches[0].clientY),t.current&&h(t.current.scrollTop),m(!0)},[]),c=H.useCallback(g=>{g.stopPropagation(),f&&m(!1)},[f]),v=H.useCallback(g=>{if(g.preventDefault(),g.stopPropagation(),f){const{current:w}=t,{scrollHeight:$,offsetHeight:C}=w,b=(g.touches[0].clientY-a)*($/C),T=Math.min(u+b,$-C);w.scrollTop=T}},[f,a,u]),y=H.useCallback(g=>{g.preventDefault(),g.stopPropagation(),s(g.clientY),t.current&&h(t.current.scrollTop),m(!0)},[]);return H.useEffect(()=>{if(t.current&&n.current){const g=t.current,{clientHeight:w}=n.current;return i.current=new ResizeObserver(()=>{k(g,w)}),i.current.observe(g),g.addEventListener("scroll",E),()=>{var $;($=i.current)==null||$.unobserve(g),g.removeEventListener("scroll",E)}}},[]),H.useEffect(()=>(f?(document.addEventListener("touchmove",v,{passive:!1}),document.addEventListener("touchend",c,{passive:!1})):(document.removeEventListener("touchmove",v),document.removeEventListener("touchend",c)),()=>{document.removeEventListener("touchmove",v),document.removeEventListener("touchend",c)}),[f,v,c]),d.jsxs(M_,{children:[d.jsx(I_,{$dragging:f,ref:t,children:e}),d.jsx(N_,{children:d.jsxs(D_,{children:[d.jsx(A_,{ref:n,onClick:x,style:{cursor:f&&"grabbing"}}),d.jsx(H_,{ref:r,onMouseDown:y,onTouchStart:p,style:{height:`${o}px`,cursor:f?"grabbing":"grab"}})]})})]})},{darkColor:$u}=de,mo={title:"Video",text:"Product Description THOTH",movies:[{id:"1",title:"Video title 1",content:"Slide 1"},{id:"2",title:"Video title 2",content:"Slide 2"},{id:"3",title:"Video title 3",content:"Slide 3"},{id:"4",title:"Video title 4",content:"Slide 4"},{id:"5",title:"Video title 5",content:"Slide 5"}]},B_=S.section`
