 ...
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */(function(e){function t(j,N){var D=j.length;j.push(N);e:for(;0<D;){var X=D-1>>>1,V=j[X];if(0<i(V,N))j[X]=N,j[D]=V,D=X;else break e}}function n(j){return j.length===0?null:j[0]}function r(j){if(j.length===0)return null;var N=j[0],D=j.pop();if(D!==N){j[0]=D;e:for(var X=0,V=j.length,bt=V>>>1;X<bt;){var Ce=2*(X+1)-1,Wt=j[Ce],Ge=Ce+1,Pt=j[Ge];if(0>i(Wt,D))Ge<V&&0>i(Pt,Wt)?(j[X]=Pt,j[Ge]=D,X=Ge):(j[X]=Wt,j[Ce]=D,X=Ce);else if(Ge<V&&0>i(Pt,D))j[X]=Pt,j[Ge]=D,X=Ge;else break e}}return N}function i(j,N){var D=j.sortIndex-N.sortIndex;return D!==0?D:j.id-N.id}if(typeof performance=="object"&&typeof performance.now=="function"){var o=performance;e.unstable_now=function(){return o.now()}}else{var l=Date,a=l.now();e.unstable_now=function(){return l.now()-a}}var s=[],u=[],h=1,f=null,m=3,S=!1,x=!1,w=!1,b=typeof setTimeout=="function"?setTimeout:null,p=typeof clearTimeout=="function"?clearTimeout:null,c=typeof setImmediate<"u"?setImmediate:null;typeof navigator<"u"&&navigator.scheduling!==void 0&&navigator.scheduling.isInputPending!==void 0&&navigator.scheduling.isInputPending.bind(navigator.scheduling);function v(j){for(var N=n(u);N!==null;){if(N.callback===null)r(u);else if(N.startTime<=j)r(u),N.sortIndex=N.expirationTime,t(s,N);else break;N=n(u)}}function g(j){if(w=!1,v(j),!x)if(n(s)!==null)x=!0,ge(_);else{var N=n(u);N!==null&&Pe(g,N.startTime-j)}}function _(j,N){x=!1,w&&(w=!1,p(C),C=-1),S=!0;var D=m;try{for(v(N),f=n(s);f!==null&&(!(f.expirationTime>N)||j&&!R());){var X=f.callback;if(typeof X=="function"){f.callback=null,m=f.priorityLevel;var V=X(f.expirationTime<=N);N=e.unstable_now(),typeof V=="function"?f.callback=V:f===n(s)&&r(s),v(N)}else r(s);f=n(s)}if(f!==null)var bt=!0;else{var Ce=n(u);Ce!==null&&Pe(g,Ce.startTime-N),bt=!1}return bt}finally{f=null,m=D,S=!1}}var k=!1,$=null,C=-1,P=5,O=-1;function R(){return!(e.unstable_now()-O<P)}function L(){if($!==null){var j=e.unstable_now();O=j;var N=!0;try{N=$(!0,j)}finally{N?z():(k=!1,$=null)}}else k=!1}var z;if(typeof c=="function")z=function(){c(L)};else if(typeof MessageChannel<"u"){var J=new MessageChannel,Y=J.port2;J.port1.onmessage=L,z=function(){Y.postMessage(null)}}else z=function(){b(L,0)};function ge(j){$=j,k||(k=!0,z())}function Pe(j,N){C=b(function(){j(e.unstable_now())},N)}e.unstable_IdlePriority=5,e.unstable_ImmediatePriority=1,e.unstable_LowPriority=4,e.unstable_NormalPriority=3,e.unstable_Profiling=null,e.unstable_UserBlockingPriority=2,e.unstable_cancelCallback=function(j){j.callback=null},e.unstable_continueExecution=function(){x||S||(x=!0,ge(_))},e.unstable_forceFrameRate=function(j){0>j||125<j?console.error("forceFrameRate takes a positive int between 0 and 125, forcing frame rates higher than 125 fps is not supported"):P=0<j?Math.floor(1e3/j):5},e.unstable_getCurrentPriorityLevel=function(){return m},e.unstable_getFirstCallbackNode=function(){return n(s)},e.unstable_next=function(j){switch(m){case 1:case 2:case 3:var N=3;break;default:N=m}var D=m;m=N;try{return j()}finally{m=D}},e.unstable_pauseExecution=function(){},e.unstable_requestPaint=function(){},e.unstable_runWithPriority=function(j,N){switch(j){case 1:case 2:case 3:case 4:case 5:break;default:j=3}var D=m;m=j;try{return N()}finally{m=D}},e.unstable_scheduleCallback=function(j,N,D){var X=e.unstable_now();switch(typeof D=="object"&&D!==null?(D=D.delay,D=typeof D=="number"&&0<D?X+D:X):D=X,j){case 1:var V=-1;break;case 2:V=250;break;case 5:V=1073741823;break;case 4:V=1e4;break;default:V=5e3}return V=D+V,j={id:h++,callback:N,priorityLevel:j,startTime:D,expirationTime:V,sortIndex:-1},D>X?(j.sortIndex=D,t(u,j),n(s)===null&&j===n(u)&&(w?(p(C),C=-1):w=!0,Pe(g,D-X))):(j.sortIndex=V,t(s,j),x||S||(x=!0,ge(_))),j},e.unstable_shouldYield=R,e.unstable_wrapCallback=function(j){var N=m;return function(){var D=m;m=N;try{return j.apply(this,arguments)}finally{m=D}}}})(Dp);Np.exports=Dp;var Wg=Np.exports;/**
 * @license React
...
`,H_=y.div`
  ...
  position: absolute;
  width: 10px;
`,F_=({children:e,...t})=>{const n=H.useRef(null),r=H.useRef(null),i=H.useRef(null),o=H.useRef(null),[l,a]=H.useState(20),[s,u]=H.useState(null),[h,f]=H.useState(0),[m,S]=H.useState(!1),[x,w]=H.useState(!1);function b(O,R){if(O){const{clientHeight:L,scrollHeight:z}=O;a(Math.max(L/z*R,20))}}const p=H.useCallback(O=>{if(O.preventDefault(),!m&&n.current&&i.current){const{scrollTop:R,scrollHeight:L}=n.current,{clientHeight:z}=r.current??{};let J=+R/+L*(z??0);J=Math.min(J,(z??0)-l);const Y=i.current;Y.style.top=`${J}px`}},[m,l]);H.useEffect(()=>{const O=n.current;if(O)return O.addEventListener("wheel",p,{passive:!1}),()=>{O.removeEventListener("wheel",p)}},[p]);const c=H.useCallback(O=>{O.preventDefault(),O.stopPropagation();const{clientY:R}=O,{clientHeight:L}=r.current,{current:z}=n;if(z){O.persist();const J=-(l/2),Y=(R-O.currentTarget.offsetTop+J)/L,ge=Math.floor(Y*z.scrollHeight);z.scrollTo({top:ge,behavior:"smooth"})}},[l]),v=H.useCallback(()=>{if(!m){if(!n.current||!r.current||!i.current)return;const{scrollTop:O,scrollHeight:R}=n.current,{clientHeight:L}=r.current;let z=+O/+R*L;z=Math.min(z,L-l);const J=i.current;J.style.top=`${z}px`}},[m,l]),g=H.useCallback(O=>{O.stopPropagation(),u(O.touches[0].clientY),n.current&&f(n.current.scrollTop),w(!0),S(!0)},[]),_=H.useCallback(()=>{x&&(w(!1),S(!1))},[x]),k=H.useCallback(O=>{if(x){const{current:R}=n,{scrollHeight:L,offsetHeight:z}=R,J=(O.touches[0].clientY-s)*(L/z),Y=Math.min(h+J,L-z);R.scrollTop=Y;const{clientHeight:ge}=r.current||{};let Pe=Y/L*(ge||0);Pe=Math.min(Pe,ge-l);const j=i.current;j&&(j.style.top=`${Pe}px`)}},[x,s,h,l]),$=H.useCallback(O=>{if(x){O.preventDefault(),O.stopPropagation();const{current:R}=n,{scrollHeight:L,offsetHeight:z}=R,J=(O.clientY-s)*(L/z),Y=Math.min(h+J,L-z);requestAnimationFrame(()=>{R.scrollTop=Y})}},[x,s,h]),C=H.useCallback(O=>{O.preventDefault(),O.stopPropagation(),u(O.clientY),n.current&&f(n.current.scrollTop),w(!0),S(!0)},[]),P=H.useCallback(O=>{O.preventDefault(),O.stopPropagation(),x&&(w(!1),S(!1))},[x]);return H.useEffect(()=>{if(n.current&&r.current){const O=n.current,{clientHeight:R}=r.current;return o.current=new ResizeObserver(()=>{b(O,R)}),o.current.observe(O),O.addEventListener("scroll",v),()=>{var L;(L=o.current)==null||L.unobserve(O),O.removeEventListener("scroll",v)}}},[]),H.useEffect(()=>(document.addEventListener("touchmove",k,{passive:!1}),document.addEventListener("touchend",_,{passive:!1}),()=>{document.removeEventListener("touchmove",k),document.removeEventListener("touchend",_)}),[k,_]),H.useEffect(()=>(document.addEventListener("mousemove",$),document.addEventListener("mouseup",P),document.addEventListener("mouseleave",P),()=>{document.removeEventListener("mousemove",$),document.removeEventListener("mouseup",P),document.removeEventListener("mouseleave",P)}),[$,P]),d.jsxs(R_,{children:[d.jsx(I_,{ref:n,...t,children:e}),d.jsx(N_,{children:d.jsxs(D_,{children:[d.jsx(A_,{ref:r,onClick:c,style:{cursor:m&&"grabbing"}}),d.jsx(H_,{ref:i,onMouseDown:C,onTouchStart:g,style:{height:`${l}px`,cursor:m?"grabbing":"grab"}})]})})]})},{darkColor:$u}=pe,mo={title:"Video",text:"Product Description THOTH",movies:[{id:"1",title:"Video title 1",content:"Slide 1"},{id:"2",title:"Video title 2",content:"Slide 2"},{id:"3",title:"Video title 3",content:"Slide 3"},{id:"4",title:"Video title 4",content:"Slide 4"},{id:"5",title:"Video title 5",content:"Slide 5"}]},B_=y.section`
